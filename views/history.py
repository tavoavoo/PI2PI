...
class HistorialView(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        # 1. Buscador de Texto
        ctk.CTkLabel(tools_frame, text="🔍 BUSCAR:", font=("Arial", 11, "bold"), text_color="gray").pack(side="left", padx=(15, 5), pady=10)
        ...
        ctk.CTkLabel(tools_frame, text="📅 IR A FECHA:", font=("Arial", 11, "bold"), text_color="gray").pack(side="left", padx=(5, 5))
    ...
    def renderizar_pagina(self):
        ...
        # ORDEN DESCENDENTE (Lo más nuevo primero)
    ...
    # --- LÓGICA DE SALTO DE FECHA ---
    def ir_a_fecha(self):
        """Calcula en qué página está una fecha y salta ahí sin filtrar."""
        ...
        try:
            ...
            # Contar cuántos registros hay DESPUÉS de esa fecha (porque el orden es DESC)
            # Si busco 08/12, quiero saltar a donde empiezan los del 08/12.
            # Los del 09/12, 10/12, 01/01/2026 están ANTES en la lista visual (páginas 1, 2...)
            
            self.c.cursor.execute("SELECT COUNT(*) FROM operaciones WHERE fecha > ?", (target_iso,))
            count_newer = self.c.cursor.fetchone()[0]
            
            # Calcular página
            # Si hay 55 items más nuevos y entran 10 por página:
            # Pag 1: 1-10, Pag 2: 11-20... Pag 6: 51-60. 
            # El item 56 (el primero de mi fecha) estará en la página 6.
            ...
            self.lbl_status.configure(text=f"📅 Saltando a fecha {date_str} (Pág {self.pagina})", text_color="#8e44ad")
    ...
    def ejecutar_sync_silencioso(self, api_key, api_secret):
        ...
        try:
            ...
            # Este código solo inserta lo que NO existe en la DB local.
            orders = self.c.fetch_binance_history(api_key, api_secret)
            if not orders:
                self.lbl_status.configure(text="✅ Todo al día.", text_color="#2cc985"); return
            ...
            for orden in orders:
                ...
                # 2. SEGURIDAD: Si está en la lista negra, ignorar.
    ...
    def modal_editar(self, oid):
        ...
        def guardar_cambios():
            try:
                ...
                if not new_banco or new_banco == "Sin Cuentas": self.c.show_error("Error", "Banco inválido."); return
            ...
            except Exception as e: self.c.show_error("Error Crítico", str(e))
    ...
    def eliminar_seleccionados(self):
        ...
        self.c.ask_confirm("Borrado Masivo", f"¿Eliminar {len(ids)} operaciones?", lambda: self.do_delete_batch(ids))
    ...
    def borrar(self, oid): self.c.ask_confirm("Borrar", f"¿Eliminar operación ID {oid}?", lambda: self.do_delete(oid))
    ...
    def pedir_credenciales(self):
        ...
        def save():
            ...
            else: self.c.show_error("Error", "Campos vacíos.")
